...
#--------------------------#
{
   ...
   # Run interproscan
   cd ${OUT_FOLDER}
   perl -pe 's/\*$//' ../${snakemake_input[reclocus_cds_aa]} > ${snakemake_params[prefix]}_reference_reclocus_CDS_aa_pfamscan.fa
   pfamscan-distributed.sh ${snakemake_params[prefix]}_reference_reclocus_CDS_aa_pfamscan.fa 600 ${snakemake_params[lsf_allocation]} 30

   # Create a check file to let snakemake know that we launched all jobs
   touch ../${snakemake_output[pfam_launched]}
